...
define('ADMIN_LAB_SEAT_CAPACITY', 50);

/** Extract seat number from labels like Seat 01, PC-02, pc 3. */
function admin_seat_number_from_name($name) {
...
function admin_seat_label($seatNum) {
    return 'Seat ' . str_pad((string)$seatNum, 2, '0', STR_PAD_LEFT);
}

function admin_preferred_lab_storage_key(array $labKeys) {
...
/** PCs in a lab for reservation assign dropdown — always Seat 01 through Seat 50. */
function admin_get_lab_computers(mysqli $conn, $lab, $capacity = ADMIN_LAB_SEAT_CAPACITY) {
    $keys = admin_lab_name_match_keys($lab);
    $bySeat = [];

    if (!empty($keys)) {
        $in = implode(',', array_fill(0, count($keys), '?'));
        $stmt = $conn->prepare("SELECT computer_name, status FROM computers WHERE lab_name IN ($in) ORDER BY computer_name ASC");
        if ($stmt) {
            $types = str_repeat('s', count($keys));
            admin_stmt_bind_params($stmt, $types, $keys);
            $stmt->execute();
            $res = $stmt->get_result();
            while ($row = $res->fetch_assoc()) {
                $seatNum = admin_seat_number_from_name($row['computer_name']);
                if ($seatNum !== null && $seatNum >= 1 && $seatNum <= $capacity) {
                    $bySeat[$seatNum] = [
                        'name' => admin_seat_label($seatNum),
                        'status' => $row['status'],
                        'db_name' => $row['computer_name'],
                    ];
                }
            }
            $stmt->close();
...
/** Lock an available PC or create the seat row when approving / starting sit-in. */
function admin_reserve_computer(mysqli $conn, $lab, $selectedComputer) {
    $selectedComputer = trim((string)$selectedComputer);
    if ($selectedComputer === '') {
        return null;
    }
    ...
    $seatNum = admin_seat_number_from_name($selectedComputer);
    $canonicalName = ($seatNum !== null && $seatNum >= 1 && $seatNum <= ADMIN_LAB_SEAT_CAPACITY)
        ? admin_seat_label($seatNum)
        : $selectedComputer;

    $pcStmt = $conn->prepare("SELECT id, computer_name, status FROM computers WHERE computer_name = ? AND lab_name IN ($labIn) LIMIT 1");
    ...
    if (!$pcData && $seatNum !== null) {
        $pcStmt = $conn->prepare("SELECT id, computer_name, status FROM computers WHERE lab_name IN ($labIn)");
        $typesAll = str_repeat('s', count($labKeys));
        admin_stmt_bind_params($pcStmt, $typesAll, $labKeys);
        $pcStmt->execute();
        $allPcs = $pcStmt->get_result();
        while ($row = $allPcs->fetch_assoc()) {
            if (admin_seat_number_from_name($row['computer_name']) === $seatNum) {
                $pcData = $row;
                break;
            }
        }
    ...
    if ($pcData) {
        ...
        $computerId = (int)$pcData['id'];
        $lockPcStmt = $conn->prepare("UPDATE computers SET status = 'in_use' WHERE id = ? AND status = 'available'");
        $lockPcStmt->bind_param('i', $computerId);
        $lockPcStmt->execute();
        $locked = $lockPcStmt->affected_rows > 0;
        $lockPcStmt->close();
        return $locked ? trim((string)$pcData['computer_name']) : null;
    }

    $storageLab = admin_preferred_lab_storage_key($labKeys);
    if ($storageLab === '') {
        return null;
    }

    $insertName = $canonicalName;
    $ins = $conn->prepare("INSERT INTO computers (lab_name, computer_name, status) VALUES (?, ?, 'in_use')");
    $ins->bind_param('ss', $storageLab, $insertName);
    if ($ins->execute()) {
        $ins->close();
        return $insertName;
    }
...
if (isset($_GET['export']) && $_GET['export'] !== '') {
    ...
    // Track assigned computer per sit-in to sync seat map status
    $checkColComputer = $conn->query("SELECT COUNT(*) as cnt FROM information_schema.COLUMNS WHERE TABLE_SCHEMA = DATABASE() AND TABLE_NAME = 'sitin_reservations' AND COLUMN_NAME = 'computer_name'");
    if ($checkColComputer && $checkColComputer->fetch_assoc()['cnt'] == 0) {
        @$conn->query("ALTER TABLE sitin_reservations ADD COLUMN computer_name VARCHAR(50) NULL AFTER lab");
    }
    ...
    if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action'])) {
        if ($_POST['action'] === 'approve_reservation') {
        ...
        } elseif ($_POST['action'] === 'toggle_reservation') {
            $enabled = isset($_POST['reservation_enabled']) && $_POST['reservation_enabled'] === '1' ? '1' : '0';
            app_set_setting($conn, 'reservation_enabled', $enabled);
            header('Location: admin_dashboard.php?settings_saved=1');
            exit();
        } elseif ($_POST['action'] === 'add_software') {
            $lab_name = trim($_POST['lab_name'] ?? '');
            $software_name = trim($_POST['software_name'] ?? '');
            $version = trim($_POST['version'] ?? '');
            if ($lab_name !== '' && $software_name !== '') {
                $stmt = $conn->prepare('INSERT INTO lab_software (lab_name, software_name, version) VALUES (?, ?, ?)');
                $stmt->bind_param('sss', $lab_name, $software_name, $version);
                $stmt->execute();
                $stmt->close();
            }
            header('Location: admin_dashboard.php?software_saved=1');
            exit();
        } elseif ($_POST['action'] === 'delete_software') {
            $swId = intval($_POST['software_id'] ?? 0);
            if ($swId > 0) {
            ...
            header('Location: admin_dashboard.php?software_deleted=1');
            exit();
        } elseif ($_POST['action'] === 'import_software_csv' && isset($_FILES['software_csv']) && $_FILES['software_csv']['error'] === UPLOAD_ERR_OK) {
            $handle = fopen($_FILES['software_csv']['tmp_name'], 'r');
            if ($handle) {
                $rowNum = 0;
                while (($data = fgetcsv($handle)) !== false) {
                    $rowNum++;
                    if ($rowNum === 1 && isset($data[0]) && stripos($data[0], 'lab') !== false) {
                        continue;
                    }
                    $lab = trim($data[0] ?? '');
                    $name = trim($data[1] ?? '');
                    ...
                    $stmt = $conn->prepare('INSERT INTO lab_software (lab_name, software_name, version) VALUES (?, ?, ?)');
                    $stmt->bind_param('sss', $lab, $name, $ver);
                    $stmt->execute();
                    $stmt->close();
                }
                fclose($handle);
            }
            header('Location: admin_dashboard.php?software_imported=1');
            exit();
        }
    }
    ...
    if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action'])) {
        if ($_POST['action'] === 'approve_sitin') {
            ...
            $labStmt->execute();
            $sitData = $labStmt->get_result()->fetch_assoc();
            $labStmt->close();

            if (!$sitData || empty($sitData['lab'])) {
            ...
            $computerName = admin_reserve_computer($conn, $lab, $selectedComputer);
            if ($computerName === null) {
                header("Location: admin_dashboard.php?lab_full=1");
                exit();
            }

            $stmt = $conn->prepare("UPDATE sitin_reservations SET status = 'approved', notified = 0, computer_name = ? WHERE id = ?");
            $stmt->bind_param("si", $computerName, $sitid);
            $stmt->execute();
            $stmt->close();
            ...
            if ($id_number && $student_name && $purpose && $lab && $selectedComputer && $remaining_sessions > 0) {
                ...
                $insertStmt = $conn->prepare("INSERT INTO sitin_reservations (id_number, student_name, purpose, lab, computer_name, remaining_sessions, status, time_in, `date`) VALUES (?, ?, ?, ?, ?, ?, 'approved', NOW(), CURDATE())");
                $insertStmt->bind_param('sssssi', $id_number, $student_name, $purpose, $lab, $computerName, $remaining_sessions);
                $insertStmt->execute();
                $insertStmt->close();
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['logout_student_id'])) {
    ...
    $sitStmt = $conn->prepare("SELECT id_number, student_name, purpose, lab, computer_name, remaining_sessions, time_in, `date` FROM sitin_reservations WHERE id = ? AND status = 'approved'");
    ...
    if ($sitDetails) {
        ...
        $time_in_val = app_resolve_sitin_time_in($sitDetails);
        $pc_name = !empty($sitDetails['computer_name']) ? $sitDetails['computer_name'] : '';
        
        // 1. Move to history table
        $historyStmt = $conn->prepare("INSERT INTO sitin_history (id_number, student_name, purpose, lab, computer_name, sessions_used, time_in, time_out, status) VALUES (?, ?, ?, ?, ?, ?, ?, NOW(), 'completed')");
        $historyStmt->bind_param("sssssis", $student_id, $sitDetails['student_name'], $sitDetails['purpose'], $sitDetails['lab'], $pc_name, $sessions_used, $time_in_val);
        $historyStmt->execute();
        $historyStmt->close();
    ...
    if ($search !== '') {
        // Update search query to include sessions and photo
...
$adminAnalytics = app_admin_analytics($conn);
$adminAiTips = app_admin_ai_recommendations($conn);
$labSoftwareResult = $conn->query('SELECT * FROM lab_software ORDER BY lab_name, software_name');
$reservationEnabled = app_reservations_enabled($conn);

// Reservation counts for dashboard
...
$rejectedCount = $conn->query("SELECT COUNT(*) as cnt FROM sitin_reservations WHERE status = 'rejected'")->fetch_assoc()['cnt'];
    ...
    <body>
     ...
     <!-- Notification Banner for Reservation Approvals -->
     <?php
     if (isset($_GET['approved']) && $_GET['approved'] == 1): ?>
         <div style="background: linear-gradient(135deg, #10b981, #059669); color: white; padding: 12px 24px; text-align: center; font-weight: 600; font-size: 15px; box-shadow: 0 4px 12px rgba(16, 185, 129, 0.3);">
...
<?php if (isset($_GET['lab_full']) && $_GET['lab_full'] == 1): ?>
     ...
     <?php endif; ?>
     <?php if (isset($_GET['session_ended']) && $_GET['session_ended'] == 1): ?>
         <div style="background: linear-gradient(135deg, #8b5cf6, #7c3aed); color: white; padding: 12px 24px; text-align: center; font-weight: 600; font-size: 15px; box-shadow: 0 4px 12px rgba(139, 92, 246, 0.3);">
             🏁 Session ended and student logged out. Sessions updated.
         </div>
     <?php endif; ?>
     <?php if (isset($_GET['announcement_posted']) && $_GET['announcement_posted'] == 1): ?>
         <div style="background: linear-gradient(135deg, #f59e0b, #d97706); color: white; padding: 12px 24px; text-align: center; font-weight: 600; font-size: 15px; box-shadow: 0 4px 12px rgba(245, 158, 11, 0.3);">
             📢 Announcement posted successfully!
         </div>
     <?php endif; ?>
     <?php if (isset($_GET['settings_saved'])): ?>
         <div style="background:#d1fae5;color:#065f46;padding:12px 24px;text-align:center;font-weight:600;">Reservation setting updated.</div>
     <?php endif; ?>
     <?php if (isset($_GET['software_saved']) || isset($_GET['software_imported']) || isset($_GET['software_deleted'])): ?>
         <div style="background:#dbeafe;color:#1e40af;padding:12px 24px;text-align:center;font-weight:600;">Software list updated.</div>
     <?php endif; ?>
     ...
     <div class="admin-layout">
        <aside class="admin-sidebar">
            ...
            <ul>
                ...
                <li><button type="button" onclick="openFeature('analyticsModal')">Analytics</button></li>
                <li><button type="button" onclick="openFeature('aiModal')">AI Recommendations</button></li>
                <li><button type="button" onclick="openFeature('softwareModal')">Software Apps</button></li>
                ...
                <li><button type="button" onclick="openFeature('reservationModal')" style="position:relative;">
                    Reservation
                    <?php 
                    $pendingCountAll = $conn->query("SELECT COUNT(*) as cnt FROM sitin_reservations WHERE status = 'pending'")->fetch_assoc()['cnt'];
                    $labResPending = $conn->query("SELECT COUNT(*) as cnt FROM reservations WHERE status = 'pending'")->fetch_assoc()['cnt'];
                    $totalPending = $pendingCountAll + $labResPending;
                    if ($totalPending > 0): ?>
                        <span style="position:absolute; top:-8px; right:-8px; background:#ef4444; color:white; border-radius:50%; width:22px; height:22px; font-size:12px; font-weight:bold; display:flex; align-items:center; justify-content:center;"><?php echo $totalPending; ?></span>
                    <?php endif; ?>
                </button></li>
        ...
        <main class="admin-main">
            ...
            <section class="dashboard-card" style="background:linear-gradient(135deg, #1e293b, #334155);">
                <h3 style="color:#fff;">🎫 Reservation Overview</h3>
                <div style="display:grid; grid-template-columns:repeat(4, 1fr); gap:15px; margin-top:10px;">
                    <div style="text-align:center; padding:15px; background:rgba(245,158,11,0.2); border-radius:12px; border:1px solid rgba(245,158,11,0.3);">
                        <div style="font-size:28px; font-weight:700; color:#f59e0b;"><?php echo $pendingCount; ?></div>
                        <div style="font-size:12px; color:#f59e0b;">Pending</div>
                    </div>
                    <div style="text-align:center; padding:15px; background:rgba(16,185,129,0.2); border-radius:12px; border:1px solid rgba(16,185,129,0.3);">
                        <div style="font-size:28px; font-weight:700; color:#10b981;"><?php echo $approvedCount; ?></div>
                        <div style="font-size:12px; color:#10b981;">Approved</div>
                    </div>
                    <div style="text-align:center; padding:15px; background:rgba(99,102,241,0.2); border-radius:12px; border:1px solid rgba(99,102,241,0.3);">
                        <div style="font-size:28px; font-weight:700; color:#6366f1;"><?php echo $completedCount; ?></div>
                        <div style="font-size:12px; color:#6366f1;">Completed</div>
                    </div>
                    <div style="text-align:center; padding:15px; background:rgba(239,68,68,0.2); border-radius:12px; border:1px solid rgba(239,68,68,0.3);">
                        <div style="font-size:28px; font-weight:700; color:#ef4444;"><?php echo $rejectedCount; ?></div>
                        <div style="font-size:12px; color:#ef4444;">Rejected</div>
                    </div>
                </div>
                <p style="margin-top:15px; font-size:14px; color:#94a3b8;">
                    <a href="#" onclick="document.getElementById('reservationModal').style.display='block'; document.body.style.overflow='hidden'; return false;" style="color:#60a5fa;">View full reservation management →</a>
                </p>
            </section>
    ...
    <div class="admin-modal" id="searchModal" style="display:none;">
     ...
     // ✅ FIXED SEARCH QUERY - INCLUDES remaining_sessions AND photo
if ($search !== '') {
    $stmt = $conn->prepare("SELECT id_number, last_name, first_name, middle_name, course, year_level, email, address, photo, COALESCE(remaining_sessions, 30) as remaining_sessions FROM students WHERE id_number LIKE ? OR last_name LIKE ? OR first_name LIKE ?");
    $like = "%{$search}%";
    $stmt->bind_param('sss', $like, $like, $like);
...
<td style="display:flex; gap:5px;">
    ...
    <div class="admin-modal" id="sitInFormModal" style="display:none;">
        <div class="sitin-form-modern" style="max-width:500px; width:95%; margin:5% auto; background:white; border-radius:20px; box-shadow:0 20px 60px rgba(0,0,0,0.3); padding:30px; position:relative;">
            ...
            foreach ($sitinLabs as $sitinLab) {
                $sitinPcsByLab[$sitinLab] = admin_get_lab_computers($conn, $sitinLab);
            }
            ?>
            <form method="POST">
    ...
    <?php include __DIR__ . '/includes/admin_extra_modals.php'; ?>
    ...
    <div class="admin-modal" id="reservationModal" style="display:none;">
        <div class="admin-modal-content" style="max-width:1100px; width:95%;">
            ...
            <form method="POST" style="display:flex;align-items:center;gap:12px;margin:12px 0 20px;padding:12px;background:#f1f5f9;border-radius:10px;flex-wrap:wrap;">
                <input type="hidden" name="action" value="toggle_reservation">
                <strong>Student reservations:</strong>
                <label style="display:flex;align-items:center;gap:6px;cursor:pointer;">
                    <input type="radio" name="reservation_enabled" value="1" <?php echo $reservationEnabled ? 'checked' : ''; ?>> Enabled
                </label>
                <label style="display:flex;align-items:center;gap:6px;cursor:pointer;">
                    <input type="radio" name="reservation_enabled" value="0" <?php echo !$reservationEnabled ? 'checked' : ''; ?>> Disabled
                </label>
                <button type="submit" style="padding:8px 14px;background:#6366f1;color:#fff;border:none;border-radius:6px;cursor:pointer;">Save</button>
            </form>
             
            <div style="display:flex; gap:10px; margin-bottom:20px; border-bottom:2px solid #e5e7eb; padding-bottom:10px;">
                <button type="button" onclick="showResTab('seatMap')" id="resTabSeatMap" style="padding:10px 20px; border:none; border-radius:8px; background:#6366f1; color:white; cursor:pointer; font-weight:600;">Seat Map</button>
                <button type="button" onclick="showResTab('computerControl')" id="resTabComputer" style="padding:10px 20px; border:none; border-radius:8px; background:#e5e7eb; color:#374151; cursor:pointer; font-weight:600;">Computer Control</button>
            ...
            <!-- SEAT MAP -->
            <div id="resSeatMap" style="display:none;">
                <?php
                $seatMapLabs = ['524', '526', '528', '530', '542', '544'];
                $seatMapCapacity = 50;
                $reservationSeatMap = [];
                ...
                ?>
                <div style="background:linear-gradient(135deg,#f8fafc,#eef2ff); border:1px solid #e2e8f0; border-radius:16px; padding:18px; margin-bottom:16px;">
                    <div style="display:flex; gap:12px; align-items:center; justify-content:space-between; flex-wrap:wrap;">
                        <div>
                            <h4 style="margin:0 0 6px 0; color:#1e293b;">🪑 Lab Seat Map</h4>
                            <p style="margin:0; color:#64748b; font-size:13px;">Clean view of vacant and occupied seats per laboratory.</p>
                        </div>
                        <div style="display:flex; align-items:center; gap:8px;">
                            <label for="resSeatMapLabSelect" style="font-weight:600; color:#334155;">Select Lab</label>
                            <select id="resSeatMapLabSelect" style="padding:10px 12px; border:1px solid #cbd5e1; border-radius:10px; background:#fff; min-width:150px;">
                                <?php foreach ($seatMapLabs as $lab): ?>
                                    <option value="<?php echo htmlspecialchars($lab); ?>">Lab <?php echo htmlspecialchars($lab); ?></option>
                                <?php endforeach; ?>
                            </select>
                        </div>
                    </div>
                ...
                <div style="display:grid; grid-template-columns:repeat(auto-fit, minmax(180px, 1fr)); gap:12px; margin-bottom:16px;">
                    <div style="background:#ecfdf5; border:1px solid #a7f3d0; border-radius:12px; padding:12px;">
                        <div style="font-size:12px; color:#065f46; margin-bottom:4px; text-transform:uppercase; letter-spacing:0.4px;">Vacant</div>
                        <div id="resSeatMapVacant" style="font-size:24px; font-weight:700; color:#047857;">0</div>
                    </div>
                    <div style="background:#fef2f2; border:1px solid #fecaca; border-radius:12px; padding:12px;">
                        <div style="font-size:12px; color:#991b1b; margin-bottom:4px; text-transform:uppercase; letter-spacing:0.4px;">Occupied</div>
                        <div id="resSeatMapOccupied" style="font-size:24px; font-weight:700; color:#dc2626;">0</div>
                    </div>
                    <div style="background:#fffbeb; border:1px solid #fde68a; border-radius:12px; padding:12px;">
                        <div style="font-size:12px; color:#92400e; margin-bottom:4px; text-transform:uppercase; letter-spacing:0.4px;">Maintenance</div>
                        <div id="resSeatMapMaintenance" style="font-size:24px; font-weight:700; color:#d97706;">0</div>
                    </div>
                    <div style="background:#eff6ff; border:1px solid #bfdbfe; border-radius:12px; padding:12px;">
                        <div style="font-size:12px; color:#1e3a8a; margin-bottom:4px; text-transform:uppercase; letter-spacing:0.4px;">Total Seats</div>
                        <div style="font-size:24px; font-weight:700; color:#1d4ed8;"><?php echo (int)$seatMapCapacity; ?></div>
                    </div>
                </div>

                <div style="display:flex; gap:14px; margin-bottom:10px; color:#475569; font-size:13px; flex-wrap:wrap;">
                    <span style="display:flex; align-items:center; gap:6px;"><span style="width:12px; height:12px; background:#22c55e; border-radius:3px; display:inline-block;"></span>Vacant</span>
                    <span style="display:flex; align-items:center; gap:6px;"><span style="width:12px; height:12px; background:#ef4444; border-radius:3px; display:inline-block;"></span>Occupied</span>
                    <span style="display:flex; align-items:center; gap:6px;"><span style="width:12px; height:12px; background:#f59e0b; border-radius:3px; display:inline-block;"></span>Maintenance</span>
                </div>

                <div id="resSeatMapGrid" style="display:grid; grid-template-columns:repeat(10, minmax(0, 1fr)); gap:10px; background:#fff; border:1px solid #e2e8f0; border-radius:14px; padding:14px; max-height:420px; overflow:auto;"></div>
            </div>
            ...
            <div id="resReservationRequest" style="display:none;">
                ...
                $today = date('Y-m-d');
                $reservationPcsByLab = [];
                foreach (['524', '526', '528', '530', '542', '544'] as $resLabCode) {
                    $reservationPcsByLab[$resLabCode] = admin_get_lab_computers($conn, $resLabCode);
                }
                ?>
                        ...
                        <tbody>
                            <?php if ($pendingReservations && $pendingReservations->num_rows > 0): ?>
                                <?php while ($r = $pendingReservations->fetch_assoc()): ?>
                                    <?php
                                    $pendingLabPcs = admin_get_lab_computers($conn, $r['lab']);
                                    $pendingAvailableCount = 0;
                                    foreach ($pendingLabPcs as $pcRow) {
                                        if ($pcRow['status'] === 'available') {
                                    ...
                                    <tr>
                                        ...
                                        <td>
                                            <form method="POST" class="reservation-approve-form">
                                                <input type="hidden" name="action" value="approve_sitin">
                                                <input type="hidden" name="sitin_id" value="<?php echo (int)$r['id']; ?>">
                                                <label style="display:block;font-size:11px;font-weight:600;color:#64748b;margin-bottom:4px;">Select PC seat</label>
                                                <select name="computer_name" class="reservation-pc-select" required
                                                    style="width:100%;min-width:170px;padding:8px 10px;border:2px solid #c7d2fe;border-radius:8px;font-size:14px;background:#fff;cursor:pointer;">
                                                    <option value="" selected disabled>— Choose seat (1–50) —</option>
                                                    <?php foreach ($pendingLabPcs as $pcRow): ?>
                                                        <?php
                                                        $pcLabel = $pcRow['name'];
                                                        if ($pcRow['status'] === 'in_use') {
                                                            $pcLabel .= ' (In use)';
                                                        } elseif ($pcRow['status'] === 'maintenance') {
                                                            $pcLabel .= ' (Maintenance)';
                                                        }
                                                        ?>
                                                        <option value="<?php echo htmlspecialchars($pcRow['name']); ?>"
                                                            <?php echo $pcRow['status'] !== 'available' ? 'disabled' : ''; ?>>
                                                            <?php echo htmlspecialchars($pcLabel); ?>
                                                        </option>
                                                    <?php endforeach; ?>
                                                </select>
                                                <small style="display:block;margin-top:4px;color:#64748b;font-size:11px;">
                                                    <?php echo $pendingAvailableCount; ?> vacant PC(s) in this lab
                                                </small>
                                                <button type="submit" style="margin-top:8px;width:100%;background:#10b981;color:white;border:none;padding:8px 12px;border-radius:6px;cursor:pointer;font-weight:600;">Approve</button>
                                            </form>
                                        </td>
                                        <td style="vertical-align:top;">
                                            <form method="POST" style="margin-top:22px;">
                                                ...
                                                <input type="hidden" name="sitin_id" value="<?php echo (int)$r['id']; ?>">
                                                <button type="submit" style="background:#ef4444;color:white;border:none;padding:8px 14px;border-radius:6px;cursor:pointer;font-weight:600;">Reject</button>
                                            </form>
                                        </td>
                                    </tr>
                                <?php endwhile; ?>
              ...
              <div id="resReservationLogs" style="display:none;">
                  ...
                  // Get recent audit logs from all status changes
                  $recentActivity = $conn->query("SELECT * FROM sitin_reservations ORDER BY created_at DESC, id DESC LIMIT 50");
                 ?>
                 ...
                 <h4 style="margin-bottom:15px;">📊 Overall Reservation Summary</h4>
                 <div style="max-height:350px; overflow:auto; margin-bottom:25px;">
                     <table>
                         <thead>
                             <tr>
                                 <th>ID</th>
                                 ...
                                 <?php while ($r = $reservationLogs->fetch_assoc()): ?>
                                     <tr>
                                         ...
                                         <td>
                                             ...
                                             <?php elseif ($r['status'] === 'cancelled'): ?><span style="color:#6b7300; font-weight:600;">Cancelled</span>
                                             <?php else: ?><span style="color:#6b7300;"><?php echo ucfirst($r['status']); ?></span><?php endif; ?>
                                         </td>
                                         <td><?php echo $r['notified'] ? 'Yes' : 'No'; ?></td>
                                     </tr>
                                  <?php endwhile; ?>
                             <?php endif; ?>
                         </tbody>
    ...
    function closeFeature(modalId) {
     ...
     function showResTab(tabName) {
        document.getElementById('resSeatMap').style.display = 'none';
        ...
        document.getElementById('resTabSeatMap').style.background = '#e5e7eb';
        document.getElementById('resTabSeatMap').style.color = '#374151';
        ...
        if (tabName === 'seatMap') {
            document.getElementById('resSeatMap').style.display = 'block';
            document.getElementById('resTabSeatMap').style.background = '#6366f1';
            document.getElementById('resTabSeatMap').style.color = 'white';
            const mapLabSelect = document.getElementById('resSeatMapLabSelect');
            if (mapLabSelect) {
                renderReservationSeatMap(mapLabSelect.value);
            }
        } else if (tabName === 'computerControl') {
    ...
    const sitinPcsByLab = <?php echo json_encode($sitinPcsByLab, JSON_HEX_TAG | JSON_HEX_APOS | JSON_HEX_QUOT | JSON_HEX_AMP); ?>;
    ...
    function refreshSitInPcOptions() {
        ...
        const pcList = Array.isArray(sitinPcsByLab[selectedLab]) ? sitinPcsByLab[selectedLab] : [];
        pcSelect.innerHTML = '';
        ...
        placeholder.textContent = '— Choose seat (1–50) —';
        ...
        for (const pc of pcList) {
            ...
            opt.disabled = pc.status !== 'available';
    ...
    function renderReservationSeatMap(labCode) {
        ...
        const occupiedEl = document.getElementById('resSeatMapOccupied');
        const vacantEl = document.getElementById('resSeatMapVacant');
        const maintenanceEl = document.getElementById('resSeatMapMaintenance');
        if (!grid || !occupiedEl || !vacantEl || !maintenanceEl) return;

        const labSeats = Array.isArray(reservationSeatMapData[labCode]) ? reservationSeatMapData[labCode] : [];
        let occupied = 0;
        let maintenance = 0;

        grid.innerHTML = '';
        for (let i = 0; i < reservationSeatMapCapacity; i++) {
            const seatNo = i + 1;
            const seatData = labSeats[i] || null;
            const status = seatData ? seatData.status : 'available';
            const label = seatData ? seatData.label : ('Seat ' + String(seatNo).padStart(2, '0'));

            if (status === 'in_use') occupied++;
            if (status === 'maintenance') maintenance++;

            const seat = document.createElement('div');
            seat.style.borderRadius = '10px';
            seat.style.padding = '10px 8px';
            seat.style.textAlign = 'center';
            seat.style.border = '1px solid #d1d5db';
            ...
            seatIndex.textContent = '#' + seatNo;
            seatIndex.style.fontSize = '10px';
            seatIndex.style.marginTop = '4px';
            seatIndex.style.opacity = '0.9';

            if (status === 'in_use') {
                seat.style.background = '#fee2e2';
                seat.style.borderColor = '#fca5a5';
                title.style.color = '#991b1b';
                seatIndex.style.color = '#991b1b';
                seat.title = label + ' - Occupied';
            } else if (status === 'maintenance') {
                seat.style.background = '#fef3c7';
                seat.style.borderColor = '#fcd34d';
                title.style.color = '#92400e';
                seatIndex.style.color = '#92400e';
                seat.title = label + ' - Maintenance';
            } else {
                seat.style.background = '#dcfce7';
                seat.style.borderColor = '#86efac';
                title.style.color = '#166534';
                seatIndex.style.color = '#166534';
                seat.title = label + ' - Vacant';
            }
        ...
        vacantEl.textContent = Math.max(0, reservationSeatMapCapacity - occupied - maintenance);
    }
